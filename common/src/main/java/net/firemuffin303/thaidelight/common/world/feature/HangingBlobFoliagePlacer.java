...
public class HangingBlobFoliagePlacer extends FoliagePlacer {
    ...
    public HangingBlobFoliagePlacer(IntProvider intProvider, IntProvider intProvider2,int height,float hangingLeavesChance,float hangingLeavesExtensionChance) {
    ...
    protected FoliagePlacerType<?> type() {
        return ModFeatures.HANGING_BLOB_FOLIAGE_PLACER.get();
    }
    ...
    protected void createFoliage(LevelSimulatedReader levelSimulatedReader, FoliageSetter foliageSetter, RandomSource randomSource, TreeConfiguration treeConfiguration, int i, FoliageAttachment foliageAttachment, int j, int k, int l) {
        for (int m = l; m >= l - j; m--) {
            ...
            if(m == l-j){
                this.placeLeavesRowWithHangingLeavesBelow(levelSimulatedReader,foliageSetter,randomSource,treeConfiguration,foliageAttachment.pos(),n,m,foliageAttachment.doubleTrunk(),this.hangingLeavesChance,this.hangingLeavesExtensionChance);
            }else{
